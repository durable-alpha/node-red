...
async function verifyDependencies(depType = "dependencies") {
  ...
  packages.forEach((package) => {
    let modulePackage = require(path.join("../packages/nm", package, "package.json"));
    ...
    dependencies.forEach((module) => {
      try {
        if (!/^@node-red\//.test(module)) {
          ...
          try {
            rootDependencies[module].should.eql(
              modulePackage[depType][module],
              `[${package}] '${module}' version mismatch. Expected '${modulePackage.dependencies[module]}' (got '${rootDependencies[module]}') in ${depType} `
            );
          } catch (err) {
            if (fixFlag) {
  ...
  if (failures.length === 0 && fixFlag) {
    ...
    packages.forEach((package) => {
      if (packageUpdates.hasOwnProperty(package)) {
        promises.push(
          fs.writeJSON(path.join(__dirname, "../packages/nm", package, "package.json"), packageUpdates[package], {
            spaces: 4
          })
        );
      }
