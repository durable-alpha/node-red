...
//
// 1. Create a copy of packages/nm/@node-red/editor-client/src/sass/colors.scss
...
(async function () {
  ...
  await fs.copy(path.join(__dirname, "../packages/nm/@node-red/editor-client/src/sass/"), workingDir);
  await fs.copyFile(parsedArgs.in, path.join(workingDir, "colors.scss"));

  const output = sass.compile(path.join(workingDir, "style-custom-theme.scss"), {
