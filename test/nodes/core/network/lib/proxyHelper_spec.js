 ...
 * @param {import('../../../../../packages/nm/@node-red/nodes/core/network/lib/proxyHelper').ProxyOptions} [options] - The options to use for getProxyForUrl
  ...
  describe("Proxy options", function () {
    describe("lowerCaseOnly:true should prevent *_PROXY being returned", function () {
      ...
      testProxyUrl(
        ...
        "http://example",
        { lowerCaseOnly: true },
        "returns empty string because `lowerCaseOnly` is set and http_proxy is not set"
      );
      testProxyUrl(
        env,
        "",
        "https://example",
        { lowerCaseOnly: true },
        "returns empty string because `lowerCaseOnly` is set and https_proxy is not set"
      );
      testProxyUrl(
        env,
        "http://upper-case-proxy",
        "http://example",
        null,
        "returns HTTP_PROXY because lowerCaseOnly is false by default"
      );
      testProxyUrl(
        env,
        "https://upper-case-proxy",
        "https://example",
        null,
        "returns HTTPS_PROXY because lowerCaseOnly is false by default"
      );
    });
    ...
    describe("favourUpperCase:false should cause *_PROXY to being used before *_proxy", function () {
      ...
      testProxyUrl(
        ...
        "http://upper-case-proxy",
        "http://example",
        { favourUpperCase: true },
        "returns HTTP_PROXY by due to `favourUpperCase`"
      );
      testProxyUrl(
        env,
        "http://lower-case-proxy",
        "http://example",
        null,
        "returns http_proxy by as it takes precedence by default"
      );
    });
    ...
    describe("When legacy mode is true, should process urls proxy in node-red <= v3.1 compatibility mode", function () {
      ...
      testProxyUrl(
        env,
        "http://http-proxy-upper",
        "http://example/5",
        { mode: "legacy" },
        'returns HTTP_PROXY "http://http-proxy-upper" because mode is "legacy"'
      );
      ...
      testProxyUrl(
        ...
        "http://example.com",
        { mode: "legacy" },
        'incorrectly returns http_proxy for "http://example.com" when mode is "legacy"'
      );
      ...
      testProxyUrl(
        ...
        { mode: "legacy" },
        'incorrectly returns http_proxy for "http://example.com:8080" when mode is "legacy"'
      );
      ...
      testProxyUrl(
        ...
        "http://example.com",
        { mode: "legacy" },
        'incorrectly returns http_proxy for "http://example.com" when no_proxy is "example.com:80" and mode is "legacy"'
      );
