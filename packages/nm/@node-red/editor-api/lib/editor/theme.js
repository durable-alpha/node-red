...
module.exports = {
  ...
  app: function () {
    ...
    if (theme.page) {
      themeContext.page.css = serveFilesFromTheme(theme.page.css, themeApp, "/css/");
      themeContext.page.scripts = serveFilesFromTheme(theme.page.scripts, themeApp, "/scripts/");
  ...
  context: async function () {
    if (activeTheme && !activeThemeInitialised) {
      ...
      if (themePlugin) {
        if (themePlugin.css) {
          const cssFiles = serveFilesFromTheme(themePlugin.css, themeApp, "/css/", themePlugin.path);
        ...
        if (themePlugin.scripts) {
          const scriptFiles = serveFilesFromTheme(themePlugin.scripts, themeApp, "/scripts/", themePlugin.path);
        ...
        if (themePlugin.page) {
          if (themePlugin.page.favicon && !theme.page.favicon) {
            const result = serveFilesFromTheme([themePlugin.page.favicon], themeApp, "/", themePlugin.path);
          ...
          if (themePlugin.page.tabicon && themePlugin.page.tabicon.icon && !theme.page.tabicon) {
            const result = serveFilesFromTheme([themePlugin.page.tabicon.icon], themeApp, "/page/", themePlugin.path);
            if (result && result.length > 0) {
        ...
        if (themePlugin.header) {
          if (themePlugin.header.image && !theme.header.image) {
            const result = serveFilesFromTheme([themePlugin.header.image], themeApp, "/header/", themePlugin.path);
            if (result && result.length > 0) {
