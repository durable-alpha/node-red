...
function start() {
  return i18n
    .registerMessageCatalog("runtime", path.resolve(path.join(__dirname, "..", "locales")), "runtime.json")
    .then(function () {
    ...
    .then(function () {
      ...
      if (settings.UNSUPPORTED_VERSION) {
        ...
        log.error(
          "* " +
            log._("runtime.unsupported_version", { component: "Node.js", version: process.version, requires: ">=18" }) +
            " *"
        );
        log.error("*****************************************************************");
...
function reinstallModules(moduleList) {
  ...
  if (settings.hasOwnProperty("autoInstallModulesRetry")) {
    log.warn(
      log._("server.deprecatedOption", { old: "autoInstallModulesRetry", new: "externalModules.autoInstallRetry" })
    );
